...
cc._RF.push(module, 'f6649XNi5BA9ZaGlXwBVAA1', 'NetWork', __filename);
    ...
    NetWork.prototype.LogJournalReport = function (errorType, data) {
        if (ConstValue_1.ConstValue.IS_EDITIONS) {
            ...
            img.src = (NetWork.isOnlineEnv ? 'https://logserver.haibian.com/statistical/?type=7&' : 'https://ceshi-statistical.haibian.com/?type=7&') +
                ...
                "&event=" + errorType +
                "&identity=1" +
                "&extra=" + JSON.stringify({ url: location, CoursewareKey: ConstValue_1.ConstValue.CoursewareKey, empty: this.GetRequest()["empty"], CoursewareName: 'wupinbianhuan', data: data });
        }
    };
    //判断是否是线上
