    ...
    LogJournalReport(errorType, data) {
        if (ConstValue.IS_EDITIONS) {
            ...
            img.src = (NetWork.isOnlineEnv ? 'https://logserver.haibian.com/statistical/?type=7&' : 'https://ceshi-statistical.haibian.com/?type=7&') +
                ...
                "&event=" + errorType +
                "&identity=1" +
                "&extra=" + JSON.stringify({ url: location, CoursewareKey: ConstValue.CoursewareKey, empty: this.GetRequest()["empty"], CoursewareName: 'guanchalifangti_169_machao', data: data });
        }
    }
}