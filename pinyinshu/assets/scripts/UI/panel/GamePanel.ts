        ...
        ball.on(cc.Node.EventType.TOUCH_CANCEL, function(e){
            ...
            if(this.bubble_none1.node.getBoundingBox().contains(this.node.convertToNodeSpaceAR(e.currentTouch._point))) {
                if(this.gunNode.getChildByName('ballNode').opacity) {
                    ...
                    let bubbleNum = parseInt(this.bubble.getChildByName('label').getComponent(cc.Label).string);
    ...
    cueAnswer() {
        for(let i = 0; i < this.pl.length; i++) {
            if(this.an.indexOf(this.pl[i]) == -1) {
                ...
                let str = this.answerArr[i].getChildByName('label').getComponent(cc.Label).string;
                if(str.length <3) {
    ...
    isRight() {
        ...
        if(rightNum == this.an.length && this.pl.length == this.an.length) {
           ...
           if(this.checkpoint >= this.checkpointsNum + 1) {
            ...
            this.closeClock();
            this.cueAnswer();
           UIHelp.showAffirmTips(2, '挑战失败！点击重置后再次挑战',this.timer,'取消','确认',function(){this.openClock();}.bind(this),function(){this.openClock()}.bind(this));
        }
    }

    chongzhiCallback() {
        UIHelp.showAffirmTips(1,'确认要重置本关，重新开始游戏吗？',-1,'取消','确认',function(){}.bind(this),this.reset.bind(this));
    }

    tijiaoCallback() {
        this.closeClock();
        UIHelp.showAffirmTips(1,'确认提交答案吗？',-1,'取消','确认',function(){this.openClock()}.bind(this),this.isRight.bind(this));
    }
