...
export class UIHelp {
    ...
    * @param type tips类型  0:内容tips   1:系统tips
    */
    public static showOverTip(type: number, str: string = "", isButton:boolean = false, btnStr:string = '', callback: Function = null, btnCallback: Function = null, endTitle?:string) {
        let overTips = UIManager.getInstance().getUI(OverTips) as OverTips;
        if (!overTips) {
            UIManager.getInstance().openUI(OverTips, null, 210, () => {
                UIHelp.showOverTip(type, str,isButton, btnStr, callback, btnCallback, endTitle);
            });
        }
        else {
            overTips.init(type, str,isButton, btnStr, callback, btnCallback, endTitle);
        }
    }
